...
async function startServer() {
  ...
  if (req.method === "POST" && req.url === "/create-razorpay-order") {
  ...
  } else if (req.method === "POST" && req.url === "/place-order-after-payment") {
    let body = "";
    req.on("data", chunk => (body += chunk));
    req.on("end", async () => {
      try {
  ...
  } else if (req.method === "POST" && req.url === "/admin/menu") {
    let body = "";
    req.on("data", chunk => (body += chunk));
    req.on("end", async () => {
      try {
  ...
  } else if (req.method === "POST" && req.url === "/add-review") {
    ...
    try {
      ...
      res.writeHead(500, { "Content-Type": "application/json" });
      return res.end(JSON.stringify({ error: "Server error loading reviews" }));
    }
  ...
  } else if (req.method === "GET" && req.url === "/admin/menu") {
    try {
      const items = await menuCollection.find().toArray();
      res.writeHead(200, { "Content-Type": "application/json" });
      return res.end(JSON.stringify(items));
  ...
  } else if (req.method === "GET" && req.url === "/admin/users") {
    try {
  ...
  } else if (req.method === "GET" && req.url === "/admin/bookings") {
    try {
  ...
  } else if (req.method === "DELETE" && req.url.startsWith("/admin/bookings/")) {
    const id = req.url.split("/").pop();
    try {
      if (!ObjectId.isValid(id)) {
        res.writeHead(400, { "Content-Type": "application/json" });
